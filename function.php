...
function randDomain(){
	$arraymail = array('yahoo.com','gmail.com','outlook.com','yahoo.co.id','icloud.com','hotmail.com'); // ubah disini buat ganti ke domain pribadimu
...
function randomUser($ua) {
    ...
    $param = "number_generate=1&gender_type=female&country=javanese,sundanese,batak,indonesian,&submit=Generate";
    $headers = array();
...
function randomUseragent(){
	...
	$androidVersion = array('10','11','12','12.1','13');
...
function getAccessToken($token){
	...
	$method = "GET";
	$headers = array();
	// $headers[] = 'User-Agent: Mozilla/5.0 (Linux; Android 13; RMX2061) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.186 Mobile Safari/537.36';
	$headers[] = 'User-Agent: Mozilla/5.0 (Linux; Android 13; SM-S918B Build/TP1A.220624.014) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Mobile Safari/537.36';
	$headers[] = 'Content-Type: application/x-www-form-urlencoded';
	$headers[] = 'Accept: */*';
	$headers[] = 'Accept-Encoding: gzip, deflate, br';
	$headers[] = 'Accept-Language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7';
...
function banner($Purple,$White){
	echo "$Purple
  ()                       
  /\          _/_   /)     
 /  )  _   __ /  o // __  ,
/__/__/_)_(_)<__<_//_/ (_/_  "."$White Agathasangkara from Membership Digital Premium 2.0 $Purple
     /           />     /  
    '           </     '   \n\n$White";
}
...
function generateAccount($Red,$White,$Green,$Grey,$Blue){
    ...
    if (empty($uname)) {
        ...
        echo "\n$Red Sudah ada akun untuk email ini.$White";
        continue;
    } else {
		var_dump("\n\n$register\n\n"); die;
	}
	...
	preg_match_all('/^Set-Cookie:\s*([^;]*)/mi', $gen, $matches);
		...
		$n = "\n";
		$m1 = substr(str_shuffle('AMNGFX'), 0, 1);
		$m2 = substr(str_shuffle('BDUFNX0'), 0, 1);
		$did = uniqueId();
		$did1 = sessionId();
		$genDevices = "SM-"."$m1".substr(str_shuffle('123456789'), 0, 3)."$m2";
		// $genDevices = "SM-F936B";
		$serial = "($genDevices)";
		...
		echo "\n\n$Green Akun berhasil dibuat!\n Email : $email\n Password : $pass ";
		$saved = '{"Email":"'.$email.'","Password":"'.$pass.'"}';
		
		file_put_contents('res_spotify.json', "$saved\n", FILE_APPEND);
	}
	// echo "\n\n$Red Gagal : $White$gagal";
	// echo "\n\n$White Password : $White$pass\n";
	echo "\n\n$White Akun tersimpan di res_spotify.json\n\n Membership Digital Premium 2.0\n Telegram : @MuhRifq\n WhatsApp : 082214199122\n Website : www.waroenginternet.com\n";
}
